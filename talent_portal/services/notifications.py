...
BRAND          = load_brand()
FROM_EMAIL     = f"{BRAND.name} Recruitment <{GMAIL_USER}>"
...
def _build_email(name: str, event_type: str, data: dict, ref_token: str = None) -> tuple[str, str]:
    ...
    header = f"""
        <div style="background:#1a1a2e;padding:20px 24px;border-radius:8px 8px 0 0">
          <span style="color:#fff;font-size:18px;font-weight:700;">{BRAND.name} — Recruitment</span>
        </div>
    """

    bodies = {
        "application_submitted": (
            f"Application Received – {BRAND.program_name}",
            f"""<p>Dear {name},</p>
            <p>Thank you for applying to the <strong>{BRAND.program_name}</strong>.
        ...
        "screening_failed": (
            ...
            f"""<p>Dear {name},</p>
            <p>Thank you for your interest in the {BRAND.program_name}.
        ...
        "assessment_available": (
            ...
            f"""<p>Dear {name},</p>
            <p>Your online assessment for the {BRAND.program_name} is now available.
        ...
        "offered": (
            ...
            <strong>{BRAND.program_name}</strong> at {BRAND.company_name}.</p>
    ...
    <div style="{base_style}">
      ...
      <div style="background:#f9f9fb;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e5e5ef">
        ...
        <p style="font-size:12px;color:#888">
          {BRAND.company_name} · Recruitment Team<br>
...
def send_otp_email(email: str, otp: str) -> bool:
    ...
    <div style="{base_style}">
      ...
      <div style="background:#f9f9fb;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e5e5ef">
        <p>Hello,</p>
        <p>Your one-time verification passcode for the {BRAND.portal_title} is:</p>
    ...
    """
    subject = f"Your Verification Code - {BRAND.portal_title}"
    status, error_message = _deliver_email(email, subject, html)
    if status != "sent":
