...
try:
    ...
    from talent_portal.services.notifications import send_notification_async, send_otp_email
    from talent_portal.services.upload import (validate_file, prepare_upload_file, enqueue_upload, get_job_status,
                                  ALLOWED_CV_MIMETYPES, ALLOWED_DOC_MIMETYPES)
    from talent_portal.services.screening import apply_screening
    from talent_portal.services.meetings import create_meeting
    from talent_portal.services.job_postings import get_job
except ImportError:
    ...
    from services.job_postings import get_job
import secrets
...
def api_apply():
    # Support multipart (file upload), urlencoded form, or JSON
    if request.is_json:
    ...
    direct_cv_upload = str(data.get("direct_cv_upload", "")).lower() == "true"
    application_source = (data.get("application_source") or "public").strip().lower()
    referral_code = (data.get("referral_code") or "").strip()
    referrer_name = (data.get("referrer_name") or "").strip()
    if application_source not in ("public", "referral", "cohort", "manual"):
        return jsonify({"error": "Invalid application source."}), 400
    if len(referral_code) > 80 or len(referrer_name) > 160:
        return jsonify({"error": "Referral details are too long."}), 400
    ...
    try:
        ...
        return jsonify({"error": "Select an available job."}), 400
    job = get_job(job_id)
    if not job or not job["accepting"]:
        return jsonify({"error": "This job is no longer accepting applications.",
                        "code": "job_closed"}), 409
    # Never trust client-supplied role or location labels.
    role = job["title"]
            ...
            if existing:
                ...
                cur.execute("""
                    ...
                    SET full_name = %s, phone_number = %s,
                        ...
                        dob = %s,
                        nysc_status = %s, role = %s, location = %s, job_id = %s,
                        application_source = %s, referral_code = %s, referrer_name = %s,
                        stage = 'applied', stage_updated_at = NOW()
                    WHERE id = %s;
                """, (full_name, phone_number, json.dumps({"house_address": house_address}), dob,
                      nysc_status, role, location, job_id, application_source, referral_code, referrer_name, candidate_id))
            else:
                import secrets
                ...
                cur.execute("""
                    INSERT INTO candidates
                        (full_name, email, phone_number, pre_test_responses,
                         dob, nysc_status, role, location, job_id, application_source, referral_code, referrer_name, stage, stage_updated_at, ref_token, cohort_id)
                    VALUES (%s, %s, %s, %s::jsonb, %s, %s, %s, %s, %s, %s, %s, %s, 'applied', NOW(), %s, %s)
                    RETURNING id;
                """, (full_name, email, phone_number,
                      json.dumps({"house_address": house_address}),
                      dob, nysc_status, role, location, job_id, application_source, referral_code, referrer_name, ref_token, cohort_id))
