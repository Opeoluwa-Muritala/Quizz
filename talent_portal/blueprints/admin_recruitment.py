...
from talent_portal.jobs.slot_generator import generate_slots
from talent_portal.services.schedules import generate_schedule_slots
...
def get_candidate(cand_id):
    ...
    with DBConnection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT id, slot_id, ratings, overall_score, recommendation,
                       notes, status, created_at, updated_at
                FROM interview_feedback
                WHERE candidate_id = %s ORDER BY updated_at DESC;
            """, (cand_id,))
            feedback = cur.fetchall()

            cur.execute("""
                SELECT id, version, currency, base_salary, start_date, expiry_date, terms,
                       approval_status, offer_status, created_at, updated_at
                FROM offers WHERE candidate_id = %s ORDER BY version DESC;
            """, (cand_id,))
            offers = cur.fetchall()
    ...
    return jsonify({
        ...
        "feedback": [{
            "id": f[0], "slot_id": f[1], "ratings": f[2],
            "overall_score": float(f[3]), "recommendation": f[4],
            "notes": f[5], "status": f[6],
            "created_at": f[7].isoformat() if f[7] else None,
            "updated_at": f[8].isoformat() if f[8] else None,
        } for f in feedback],
        "offers": [{
            "id": o[0], "version": o[1], "currency": o[2], "base_salary": float(o[3]),
            "start_date": o[4].isoformat(), "expiry_date": o[5].isoformat(), "terms": o[6],
            "approval_status": o[7], "offer_status": o[8],
            "created_at": o[9].isoformat() if o[9] else None, "updated_at": o[10].isoformat() if o[10] else None,
        } for o in offers],
        "stage_history": [{
            "from": h[0], "to": h[1],
...
@admin_rec.route("/api/admin/recruitment/candidates/<int:cand_id>/feedback", methods=["POST"])
def save_interview_feedback(cand_id):
    err = _require_admin()
    if err:
        return err
    payload, errors = validate_feedback(request.json or {})
    if errors:
        return jsonify({"error": "Review the scorecard fields.", "fields": errors}), 400
    ...
    with DBConnection() as conn:
        with conn.cursor() as cur:
            ...
            if not candidate:
                return jsonify({"error": "Candidate not found."}), 404
            if candidate[0] != "interview_completed":
                return jsonify({"error": "Feedback is available after the interview is completed."}), 409
            cur.execute("""
                SELECT id FROM generated_slots
                WHERE id = %s AND candidate_id = %s;
            """, (payload["slot_id"], cand_id))
            if not cur.fetchone():
                return jsonify({"error": "That interview slot does not belong to this candidate."}), 400

            overall = sum(payload["ratings"].values()) / len(payload["ratings"])
            cur.execute("""
                SELECT id FROM interview_feedback
                WHERE candidate_id = %s AND slot_id IS NOT DISTINCT FROM %s;
            """, (cand_id, payload["slot_id"]))
            existing = cur.fetchone()
            params = (json.dumps(payload["ratings"]), overall, payload["recommendation"], payload["notes"])
            if existing:
                cur.execute("""
                    UPDATE interview_feedback
                    SET ratings = %s::jsonb, overall_score = %s, recommendation = %s,
                        notes = %s, status = 'submitted', updated_at = NOW()
                    WHERE id = %s RETURNING id;
                """, params + (existing[0],))
            else:
                cur.execute("""
                    INSERT INTO interview_feedback
                        (candidate_id, slot_id, ratings, overall_score, recommendation, notes, status)
                    VALUES (%s, %s, %s::jsonb, %s, %s, %s, 'submitted')
                    RETURNING id;
                """, (cand_id, payload["slot_id"], json.dumps(payload["ratings"]), overall,
                       payload["recommendation"], payload["notes"]))
            feedback_id = cur.fetchone()[0]
        conn.commit()
    return jsonify({"status": "success", "feedback_id": feedback_id, "overall_score": round(overall, 2)})


@admin_rec.route("/api/admin/recruitment/candidates/<int:cand_id>/offers", methods=["POST"])
def create_offer(cand_id):
    err = _require_admin()
    if err: return err
    payload, errors = validate_offer(request.json or {})
    if errors: return jsonify({"error": "Review the offer fields.", "fields": errors}), 400
    with DBConnection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT stage FROM candidates WHERE id=%s", (cand_id,))
            row = cur.fetchone()
            if not row: return jsonify({"error": "Candidate not found."}), 404
            if row[0] not in ("interview_completed", "offered"): return jsonify({"error": "An offer can only be created after interview completion."}), 409
            cur.execute("SELECT COALESCE(MAX(version),0)+1 FROM offers WHERE candidate_id=%s", (cand_id,))
            version = cur.fetchone()[0]
            cur.execute("""INSERT INTO offers(candidate_id,version,currency,base_salary,start_date,expiry_date,terms)
                VALUES(%s,%s,%s,%s,%s,%s,%s) RETURNING id""", (cand_id,version,payload["currency"],payload["base_salary"],payload["start_date"],payload["expiry_date"],payload["terms"]))
            offer_id = cur.fetchone()[0]
            cur.execute("INSERT INTO offer_versions(offer_id,version,snapshot) VALUES(%s,%s,%s::jsonb)", (offer_id,version,json.dumps(payload)))
        conn.commit()
    return jsonify({"status":"success", "offer_id":offer_id, "version":version})


@admin_rec.route("/api/admin/recruitment/offers/<int:offer_id>/status", methods=["POST"])
def update_offer_status(offer_id):
    err = _require_admin()
    if err: return err
    status = str((request.json or {}).get("status") or "").strip()
    if status not in OFFER_STATUSES: return jsonify({"error":"Invalid offer status."}), 400
    with DBConnection() as conn:
        with conn.cursor() as cur:
            cur.execute("UPDATE offers SET offer_status=%s, updated_at=NOW() WHERE id=%s RETURNING candidate_id", (status,offer_id))
            row = cur.fetchone()
            if not row: return jsonify({"error":"Offer not found."}), 404
        conn.commit()
    return jsonify({"status":"success", "offer_status":status})


@admin_rec.route("/api/admin/recruitment/candidates/<int:cand_id>/onboarding", methods=["GET", "POST"])
def onboarding_handoff(cand_id):
    err = _require_admin()
    if err: return err
    with DBConnection() as conn:
        with conn.cursor() as cur:
            ...
            if request.method == "POST":
                ...
                if status not in ("pending", "in_progress", "complete"): return jsonify({"error":"Invalid onboarding status."}), 400
                start_date = data.get("start_date") or None
                if start_date:
                    from datetime import date
                    try: date.fromisoformat(start_date)
                    except ValueError: return jsonify({"error":"Start date must use YYYY-MM-DD."}), 400
                owner = str(data.get("owner") or "").strip()
                notes = str(data.get("notes") or "").strip()
                if len(owner) > 160 or len(notes) > 3000: return jsonify({"error":"Owner or notes are too long."}), 400
                cur.execute("""INSERT INTO onboarding_handoffs(candidate_id,start_date,owner,status,notes)
                    VALUES(%s,%s,%s,%s,%s) ON CONFLICT(candidate_id) DO UPDATE SET start_date=EXCLUDED.start_date,
                    owner=EXCLUDED.owner,status=EXCLUDED.status,notes=EXCLUDED.notes,updated_at=NOW() RETURNING id""", (cand_id,start_date,owner,status,notes))
                handoff_id = cur.fetchone()[0]
                for label in (data.get("tasks") or []):
                    label = str(label).strip()
                    if label and len(label) <= 160: cur.execute("INSERT INTO onboarding_tasks(handoff_id,label) VALUES(%s,%s) ON CONFLICT DO NOTHING", (handoff_id,label))
            cur.execute("""SELECT h.id,h.start_date,h.owner,h.status,h.notes,
                COALESCE(json_agg(json_build_object('id',t.id,'label',t.label,'complete',t.complete) ORDER BY t.id) FILTER (WHERE t.id IS NOT NULL),'[]')
                FROM onboarding_handoffs h LEFT JOIN onboarding_tasks t ON t.handoff_id=h.id
                WHERE h.candidate_id=%s GROUP BY h.id""", (cand_id,))
            row = cur.fetchone()
        conn.commit()
    if not row: return jsonify({"handoff": None})
    return jsonify({"handoff":{"id":row[0],"start_date":row[1].isoformat() if row[1] else None,"owner":row[2],"status":row[3],"notes":row[4],"tasks":row[5]}})
...
def set_candidate_stage(cand_id):
    ...
    if new_stage == "rejected" and (not reason or len(reason) > 500):
        return jsonify({"error": "A rejection reason is required and must be 500 characters or fewer."}), 400
    if not reason:
    ...
    auto_notify = False
    if new_stage in STAGE_EMAIL_EVENT:
        with DBConnection() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT mode FROM notification_settings WHERE notification_type=%s", (STAGE_EMAIL_EVENT[new_stage],))
                setting = cur.fetchone()
        auto_notify = bool(setting is None or setting[0] == "auto")
    if notify and auto_notify and new_stage in STAGE_EMAIL_EVENT:
...
def update_slot(slot_id):
    ...
    with DBConnection() as conn:
        with conn.cursor() as cur:
            ...
            if time_changing:
                # Run overlap check
                from talent_portal.db import check_interviewer_overlap
    ...
    if is_booked and time_changing and confirm_reschedule and candidate_id:
        try:
            from zoneinfo import ZoneInfo
            from talent_portal.services.notifications import send_notification_async
