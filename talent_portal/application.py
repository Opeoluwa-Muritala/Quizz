...
def init_db():
    with DBConnection() as conn:
        with conn.cursor() as cur:
            ...
            if cur.fetchone()[0] == 0:
                from talent_portal.defaults import PASS_MARK_PERCENT, SECONDS_PER_QUESTION
                cur.execute("""
            ...
            if cur.fetchone()[0] == 0:
                from talent_portal.defaults import QUESTIONS
                for q in QUESTIONS:
                    cur.execute("""
            ...
            if cur.fetchone()[0] == 0:
                from talent_portal.defaults import WHITELIST_SEEDS
        ...
        with DBConnection() as conn:
            with conn.cursor() as cur:
                ...
                if rows:
                    ...
                    funnel['total'] = 0
                    for row in rows:
                        s, count = (row[0] or 'screening').lower(), row[1]
                        funnel['total'] += count
                        if 'screen' in s or 'app' in s:
                            funnel['screening'] += count
                        elif 'assess' in s or 'test' in s or 'quiz' in s:
                            funnel['assessment'] += count
                        elif 'interview' in s:
                            funnel['interview'] += count
                        elif 'doc' in s or 'upload' in s:
                            funnel['documents'] += count
                        else:
                ...
                for r in cur.fetchall():
                    recent_candidates.append({
                        ...
                        'name': r[1],
                        'email': r[2],
                        'role': r[3] or 'General Role',
                        'stage': (r[4] or 'screening').replace('_', ' ').capitalize(),
                ...
                if active_jobs:
                    funnel['active_jobs'] = len(active_jobs)
    except Exception as e:
        app.logger.warning(f"Error gathering admin dashboard metrics: {e}")
...
def admin_login():
    if request.method == 'GET':
        ...
        session.pop('admin', None)
        session.pop('admin_session_token', None)
        return render_template('admin/admin.html', authenticated=False)
...
def _bootstrap():
    ...
    if os.environ.get("RUN_DB_MIGRATIONS", "false").lower() == "true":
        try:
            ...
            init_recruitment_db()
            from talent_portal.schema.job_postings import init_job_postings_db
    ...
    try:
        from talent_portal.blueprints.recruitment import recruitment
        from talent_portal.blueprints.admin_recruitment import admin_rec
        from talent_portal.blueprints.job_postings import job_postings
...
def job_expire_deadlines():
    ...
    if secret and not hmac.compare_digest(secret.encode(), provided.encode()):
        return jsonify({"error": "Unauthorized"}), 401
    from talent_portal.jobs.deadline_expiry import expire_past_deadlines
